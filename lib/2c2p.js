...
class TwoC2PService {
  constructor(config = {}) {
    ...
    this.apiUrl = config.apiUrl || process.env['2C2P_API_URL'] || 'https://sandbox-pgw.2c2p.com/payment/4.3/paymentToken';
  ...
  async requestPaymentToken(params, baseUrl) {
    ...
    try {
      ...
      console.error('[2C2P API Error]', error.response?.data || error.message);
