...
function getMerchantSigningPrivate() {
  initPemKeys();
  const raw = process.env.PACO_MERCHANT_SIGNING_PRIVATE || fs.readFileSync(PATHS.merchantSigningPrivate, 'utf8');
  return raw.replace(/\\n/g, '\n');
}

function getMerchantEncryptionPrivate() {
  initPemKeys();
  const raw = process.env.PACO_MERCHANT_ENCRYPTION_PRIVATE || fs.readFileSync(PATHS.merchantEncryptionPrivate, 'utf8');
  return raw.replace(/\\n/g, '\n');
}

function getPacoPublicEncryption() {
  initPemKeys();
  const raw = process.env.PACO_PUBLIC_ENCRYPTION || fs.readFileSync(PATHS.pacoPublicEncryption, 'utf8');
  return raw.replace(/\\n/g, '\n');
}

function getPacoPublicSigning() {
  initPemKeys();
  const raw = process.env.PACO_PUBLIC_SIGNING || fs.readFileSync(PATHS.pacoPublicSigning, 'utf8');
  return raw.replace(/\\n/g, '\n');
}
