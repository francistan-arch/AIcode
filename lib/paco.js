...
class PacoService {
   ...
   * Strictly aligned with official 2C2P PACO demo module (cty: JWT, kid in JWS header, SignJWT builder)
   */
  async buildJweRequest(requestBody) {
    const unixNow = Math.floor(Date.now() / 1000);

    // 1. JWS Sign using SignJWT builder (PS256)
    const privateSigningKey = await jose.importPKCS8(this.merchantSigningPrivatePem, 'PS256');
    const jws = await new jose.SignJWT({ request: requestBody })
      .setProtectedHeader({ alg: 'PS256', typ: 'JWT', kid: 'merchant-signing-key' })
      .setIssuer(this.apiKey)
      .setAudience('PacoAudience')
      .setIssuedAt(unixNow)
      .setNotBefore(unixNow)
      .setExpirationTime(unixNow + 300)
  ...
  async decryptAndVerifyJweResponse(jweResponseText) {
    ...
    try {
      ...
      const { payload } = await jose.jwtVerify(innerJws, publicSigningKey, {
