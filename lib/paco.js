    ...
    if (this.mode === 'simulator') {
      ...
      const responsePayload = {
        ...
        availablePaymentMethods: [
          { code: 'CREDIT_CARD', name: 'Credit / Debit Card (Visa, Mastercard, JCB, UnionPay)', icon: '💳' },
          { code: 'PROMPTPAY', name: 'PromptPay / PayNow QR Code', icon: '📲' },
          { code: 'E_WALLET', name: 'E-Wallets (TrueMoney, ShopeePay, LINE Pay)', icon: '👛' },
          { code: 'BANK_TRANSFER', name: 'Online Direct Bank Transfer & FPX', icon: '🏦' }
        ],
        smartRouteInfo: {
          recommendedAcquirer: 'PACO_ACQUIRER_SG_01',
          feeSavings: '0.85%'
        }
      };
    ...
    try {
      ...
      return {
        ...
        availablePaymentMethods: decodedResponse.availablePaymentMethods || [
          ...
          { code: 'PROMPTPAY', name: 'PromptPay QR', icon: '📲' },
          { code: 'E_WALLET', name: 'E-Wallets', icon: '👛' }
        ]
      };
    } catch (error) {
