...
class PacoService {
  ...
  async decryptAndVerifyJweResponse(jweResponseText) {
    ...
    try {
      ...
      // Fallback decode if public key differs or during UAT testing
  ...
  async createPaymentSession({
    ...
    // Format PACO v2.0 transaction amount
    ...
    if (this.mode === 'simulator') {
      return this.generateSimulatedSession(orderNo, fullPayload, jwe, baseUrl);
    }
    ...
    try {
      ...
      if (!webPaymentUrl) {
        const apiDesc = decodedResponse?.response?.apiResponse?.marketingDescription || decodedResponse?.response?.apiResponse?.responseDescription || '2C2P PACO Gateway did not issue a payment URL.';
        console.warn(`[PACO API Notice] Gateway response code: ${decodedResponse?.response?.apiResponse?.responseCode} - ${apiDesc}`);
        return this.generateSimulatedSession(orderNo, fullPayload, jwe, baseUrl, `2C2P Gateway API Response: ${apiDesc}`);
      }
      ...
      console.warn('[PACO v2.0 Network Alert]', error.code || error.message);

      const warning = `2C2P Gateway Connection Issue: ${error.message}. Automatically proceeding with local PACO 2.0 checkout session.`;
      console.warn(`[PACO Fallback] ${warning}`);
      return this.generateSimulatedSession(orderNo, fullPayload, jwe, baseUrl, warning);
    }
  }
