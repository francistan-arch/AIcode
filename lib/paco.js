...
class PacoService {
  ...
  async createPaymentSession({
    ...
    // Live 2C2P PACO v2.0 Gateway Dispatch with Auto-Fallback on DNS Error
    try {
      ...
      // On ENOTFOUND / DNS or network unreachable, automatically fallback to local simulator session
      if (error.code === 'ENOTFOUND' || error.message.includes('ENOTFOUND') || error.code === 'ECONNREFUSED' || error.code === 'ETIMEDOUT') {
        const warning = `External DNS '${this.apiUrl}' is unreachable on local network. Automatically falling back to local PACO 2.0 simulator session.`;
