...
class PacoService {
  ...
  async createPaymentSession({
    ...
    try {
      ...
      if (error.code === 'ENOTFOUND' || error.message.includes('ENOTFOUND') || error.code === 'ECONNREFUSED' || error.code === 'ETIMEDOUT') {
        const warning = `External DNS '${this.apiUrl}' is unreachable. Automatically falling back to local PACO 2.0 simulator session.`;
        console.warn(`[PACO Fallback] ${warning}`);
        return this.generateSimulatedSession(orderNo, fullPayload, jwe, baseUrl, warning);
      }

      throw new Error(`2C2P PACO v2.0 Request Failed: ${typeof error.response?.data === 'object' ? JSON.stringify(error.response?.data) : (error.response?.data || error.message)}`);
    }
  }
